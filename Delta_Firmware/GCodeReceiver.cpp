...
void GCodeReceiverClass::Init(vector<String>* gCodeQueue, HardwareSerial* serial, unsigned long baudrate)  // （G 代码，串口号，波特率）  初始化，准备好接收G代码
{
	...
	this->GCodeQueue = gCodeQueue;  //  接收到的 gCodeQueue 全都传入 GCodeQueue
}

void GCodeReceiverClass::Execute()  //  执行 接收G代码
{
	...
	{
		GCodeQueue->push_back(receiveString);   // 将 G 代码 传给 receiveString
		receiveString = "";
		isStringComplete = false;
		return;
	}
