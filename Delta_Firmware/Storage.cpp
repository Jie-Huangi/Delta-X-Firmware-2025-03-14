...
#include "Storage.h"  // 向 Arduino 写入数据

void StorageClass::init()  // 初始化 类的成员函数
{
...
{	
	// 这是一个 Arduino EEPROM 中按数据类型写入的程序
	EEPROM.put(VELICITY_ADDRESS, Data.Velocity);  //将 Data.Velocity 写入 EEPROM 的 VELICITY_ADDRESS 地址中
	// EEPROM.put() 是 C++ 中的一个函数，它允许我们向 EEPROM 写入多字节的数据，包括浮点型数据、整数型数据和其他数据类型
	...
	EEPROM.put(MM_PER_ARC_SEGMENT_ADDRESS, Data.MMPerArcSegment);   // 这一句对应storage.h中的 #define MM_PER_ARC_SEGMENT_ADDRESS 76
