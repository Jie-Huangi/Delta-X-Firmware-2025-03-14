...
void MultiServoClass::CreateServo(AXIS axisname, uint8_t pin)   // 创建舵机
{
	Servo_t buffer;
...
void MultiServoClass::AddAngle(AXIS axisname, uint8_t angle)
{
	for (uint8_t i = 0; i < NumberOfServoCreated; i++)
	{
		...
		{
			ServoArray[i].Desired = angle;    // 这里将角度值赋值给了对应的舵机
			break;
		}	
	}
}

void MultiServoClass::Running()    // 舵机开始运行
{
	for (uint8_t i = 0; i < NumberOfServoCreated; i++)
	{
		if (ServoArray[i].lastPosition != ServoArray[i].Desired)
		{
			ServoArray[i].lastPosition = ServoArray[i].Desired;   // 这里就是  Desired == lastPosition

			ServoArray[i].servo.write(ServoArray[i].lastPosition);  // 这里就开始写入进去的
		}
	}
}
