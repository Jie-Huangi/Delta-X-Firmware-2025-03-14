...
bool ToolClass::CheckingDesiredPoint(Point point)
{
	...
	if (sqrt(pow(point.X, 2) + pow(point.Y, 2)) > Data.MOVING_AREA_LARGEST_DIAMETER) // 这里检查，如果超出了这个范围，就返回false
	
		return false;

	Data.DesiredPoint = point;   // 这里赋值了，相当于 将point 赋值给了 Data.DesiredPoint

	return true;
}

bool ToolClass::CheckingDesiredAngle(Angle angle)  // 检查角度范围
{
	if (angle.Theta1 < THETA1_HOME_POSITION)  // 如果超出了 规定角度，那么久返回一个false
		return false;
...
Point ToolClass::GetPointInLine(Point currentP, Point desiredP, float t)  // 得到每个插值点的坐标
{
	Point buffer;
...
float ToolClass::CalDistance2Point(Point point1, Point point2) // 空间距离函数
{
	float x_Offset = point1.X - point2.X;
	float y_Offset = point1.Y - point2.Y;
	float z_Offset = point1.Z - point2.Z;

	float distance = sqrt(pow(x_Offset, 2) + pow(y_Offset, 2) + pow(z_Offset, 2)); // x, y, z 距离，然后开根号。
