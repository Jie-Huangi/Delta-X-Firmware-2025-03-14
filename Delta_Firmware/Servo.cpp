...
void Servo::writeMicroseconds(int value)
{
	...
	if (this->servoPin == 11)
	{
		OCR1A = 0x0;   // 定时器1 的输出比较寄存器 A（OCR1A）设置为 0x0（即0），这将使与该定时器关联的 PWM 引脚停止输出电压
		OCR1A = value * 2;  //  OCR1A 设置为 value * 2，这将确定该 PWM 引脚输出的脉冲宽度，从而控制伺服电机的位置
	}
