...
#if defined(ARDUINO) && ARDUINO >= 100  // 逻辑与运算
	#include "arduino.h"
#else
	#include "WProgram.h"
#endif
...
class DeltaKinematicsClass
{
 ...
 public:
	void init();
	bool ForwardKinematicsCalculations(Angle angleposition, Point &point);  // 正解
	bool InverseKinematicsCalculations(Point point, Angle &angleposition);  // 逆解

private:
	bool AngleThetaCalculations(float x0, float y0, float z0, float &theta); // 角度计算
	float _y0_;
	float _y1_;
	float RD_RF_Pow2;
	float RD_RE_Pow2;
