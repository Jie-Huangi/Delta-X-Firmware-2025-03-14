...
#define COMPARE_VALUE_TIMER5 OCR5A     // 这里就是用于比较 timer5 的值
#define COMPARE_VALUE_TIMER2 OCR2A     // 这里就是用于比较 timer2 的值

#define TurnOnTimer2 (TIMSK2 |= (1 << OCIE2A))    // 打开计时器2
#define TurnOffTimer2 (TIMSK2 &= ~(1 << OCIE2A))    // 关闭计时器2

#define TurnOnTimer5 (TIMSK5 |= (1 << OCIE5A))    // 打开计时器5
#define TurnOffTimer5 (TIMSK5 &= ~(1 << OCIE5A))    //关闭计时器5
...
typedef struct  // 电机结构体
{
	...
	AXIS Name;    // AXIS 是一个枚举 ，里面有每个很多轴，代表每个电机
	float NumberInterrupt2Step;
}Motor;

class StepperClass  // 步进电机类别
{
 protected:
 ...
 private:
	 Motor ThetaStepMotor[3];     // 角度步进电机

	 Motor ExtrustionStepMotor;   // 挤出步进电机
