...
#include "Stepper.h"  // 运行步进电机

void StepperClass::init(vector<Segment>* SegmentQueue)   // 初始化整个程序
{
	...
	CreateMotor();    // 初始化创建的电机

	IsRunningHome = false;
	IsStoppedMoving = true;
}

void StepperClass::CreateMotor()   // 创建电机
{
...
void StepperClass::UpdateMoveSegment()  // 更新movesegment
{
...
void StepperClass::Running()     // 启动步进电机
{	
	if (SegmentQueue->size() == 0)
	{
		return;
	...
	CurrentMoveSegment = SegmentQueue->operator[](0);
	//在C++ 中的意思是将SegmentQueue容器的第一个元素赋值给CurrentMoveSegment

	SegmentQueue->erase(SegmentQueue->begin());
	//SegmentQueue->erase(SegmentQueue->begin()); 在C++ 中的意思是删除SegmentQueue容器的第一个元素。
	//SegmentQueue是一个指向容器对象的指针，erase是一个成员函数，用于删除容器中的元素。SegmentQueue->begin()是一个迭代器，指向容器的第一个元素。
	//因此，SegmentQueue->erase(SegmentQueue->begin())就相当于调用erase函数，传入第一个元素的位置作为参数，从而删除它。

	for (uint8_t i = 0; i < 3; i++)
	{
		setMotorDirection(ThetaStepMotor[i].Name, CurrentMoveSegment.StepperArray[i].Direction);  // 这里的 Direction 可能与 fastio.h 相关
		// 设置步进电机方向
		...
		// segbuffer.NumberINT = roundf(timeMove * TIME_TO_NUMINT);   //  取整  在Planner.h里面
		// segbuffer.StepperArray[i].StepsToJump = STEP_NULL;        // #define STEP_NULL 65530; 在Planner.h里面
	}

	if (Data.End_Effector == USE_PRINTER)
	{
		WRITE(EXTRUSDER_DIRECTION_PIN, Planner.ExtrustionStepsDirection);   // 挤出步进电机 旋转方向
	...
	CurrentCycle = Planner.BeginEndIntCycle;   // 当前周期
...
void StepperClass::Isr_Execute_Velocity()  // 速度
{
	// Movinghome **********
	if (IsRunningHome > 0)
	{
	...
	for (uint8_t i = 0; i < 3; i++)
	{
		...
		if (CurrentMoveSegment.StepperArray[i].StepsToJump != 0)
		{
			...
			writePulsePin(ThetaStepMotor[i].Name, 1);       //  这相当于一直给脉冲中
		}
	}

	if (Data.End_Effector == USE_PRINTER)      // 使用打印头
	{
		if (ExtrustionStepMotor.InterruptNumberNextStep < TotalTnterruptNumber)
		{
			...
			if (Planner.ExtrustionStepsToJump != 0)
			{
				...
				WRITE(EXTRUSDER_PULSE_PIN, HIGH);   // 这里给 打印头 发了 脉冲
			}
		}
	}

	TurnOnTimer2;     // 这里就是在用 时钟2 给步进电机发脉冲 // 这里又打开了 时钟 2 
...
void StepperClass::Isr_Execute_Movinghome() // 回到原点
{
...
void StepperClass::Isr_Turn_Pulse_Pin()  // 脉冲
{
	for (uint8_t i = 0; i < 3; i++)
	{
		writePulsePin(ThetaStepMotor[i].Name, 0);  // ThetaStepMotor[i].Name 这个电机的脉冲写入 0 
	}

	if (Data.End_Effector == USE_PRINTER)
	{
		WRITE(EXTRUSDER_PULSE_PIN, LOW);   // 这里使用 计时器 2 持续发脉冲；我可以使用这个步进电机，作为旋转角度或者输送带
	}
	TurnOffTimer2;
}
...
void StepperClass::SetIntCycle(float intCycle)
{
	...
	if (intCycle > 4000)      // sets the Timer/Counter Control Register
	{
		TCCR5B |= (1 << CS51);
		TCCR5B &= ~(1 << CS50);
		prescaler = 8;       // 设置预分频器
	...
	COMPARE_VALUE_TIMER5 = roundf(intCycle * F_CPU / (1000000.0 * prescaler)) - 1;   // 取整   F_CPU is the clock frequency of the microcontroller
}

void StepperClass::ClearMotorArray()   // 清空 电机列表
{
...
#ifdef USING_STEPER_FOR_AXIS4     // 这里预留了 axis 4 的程序
	case AXIS_4:
...
#ifdef USING_STEPER_FOR_AXIS5     // 这里预留了 axis 5 的程序
	case AXIS_5:
...
void StepperClass::setMotorDirection(AXIS axisname, bool variation)   // 设置 步进电机的旋转方向
{
	switch (axisname)
	{
	case THETA_1:
...
void StepperClass::writePulsePin(AXIS axisname, bool ison)  // 写入脉冲 到对应引脚（pin）
{
	switch (axisname)
	{
	case THETA_1:
...
void StepperClass::DisanableStepper()   // 步进电机 disenable
{
...
void StepperClass::EnableStepper()   // 步进电机使能
{
	WRITE(THETA1_ENABLE_PIN, LOW);  // 这里是写入 LOW（低） 电平，启动电机的意思
	WRITE(THETA2_ENABLE_PIN, LOW);
	WRITE(THETA3_ENABLE_PIN, LOW);
	if (Data.End_Effector == USE_PRINTER)
	{
...
ISR(TIMER5_COMPA_vect)   // timer5 vect
{
	Stepper.Isr_Execute_Velocity();
}

ISR(TIMER2_COMPA_vect)   // timer2 vect
{
