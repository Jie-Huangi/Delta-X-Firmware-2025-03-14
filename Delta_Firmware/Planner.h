...
#define TIME_TO_US 1000000.0   // 时间
...
typedef struct  // 定义step结构
{
	uint16_t StepsToJump;
	bool Direction;   // false  || true
}Step;     // step 里面有 整型 StepsToJump 和 布尔型 Direction

class Segment  //定义segment（插值）类
{
public:
	Step StepperArray[3];  // 步进电机列表
	uint32_t NumberINT;
};

class PlannerClass  //定义路径规划类
{
 protected:
