...
{
#if THETA1_ENDSTOP_INVERT != false   // x endstop
	pinMode(THETA1_ENDSTOP_PIN, INPUT);
#else
	pinMode(THETA1_ENDSTOP_PIN, INPUT_PULLUP);  // 输入上拉模式
#endif // THETA1_ENDSTOP_INVERT

#if THETA2_ENDSTOP_INVERT != false  // y endstop
	pinMode(THETA2_ENDSTOP_PIN, INPUT);
#else
	pinMode(THETA2_ENDSTOP_PIN, INPUT_PULLUP);
#endif // THETA1_ENDSTOP_INVERT

#if THETA3_ENDSTOP_INVERT != false  // z endstop
...
#if ENDSTOP_FOR_CHECK_Z_AREA_INVERT != false   // z endstop
...
void EndStopsClass::UpdateStates()  // 更新机械臂状态
{
...
bool EndStopsClass::HomingCheck()  // 检查是否回到原点
{
...
void EndStopsClass::ReportStates()  // 显示是否回到原点
{
	UpdateStates();  // 这里更新机械臂的状态
...
{
	switch (axis)  // 相当于if else if else 。只是这里的结果取决于axis的值，axis等于几，程序就执行对应的case语句。
	{
	case THETA_1:
		return Theta1State;
		break;
...
void EndStopsClass::ClearEndstopState()  // 清除endstop状态
{
	Theta1State = false;
	Theta2State = false;
	Theta3State = false;
